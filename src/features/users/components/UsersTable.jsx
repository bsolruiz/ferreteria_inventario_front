...
export default function UsersTable({
  ...
  const handleEliminar = async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.mensaje || "Error al desactivar usuario";
      setError(msg);
    } finally {
  ...
  const handleActivar = async () => {
    if (!usuarioActivando) return;

    setLoadingActivate(true);

    try {
      const datosActualizar = {
        nombres: usuarioActivando.nombres,
        correo: usuarioActivando.correo,
        rolId: usuarioActivando.rolId,
        estado: 1,
      };
      const actualizado = await actualizarUsuario(
        usuarioActivando.idUsuario,
        datosActualizar,
      );
      onActualizado({
        ...usuarioActivando,
        ...datosActualizar,
        ...(typeof actualizado === "object" ? actualizado : {}),
      });
      toast.success("Usuario activado correctamente");
      setUsuarioActivando(null);
    } catch (err) {
      toast.error(err.response?.data?.mensaje || "Error al activar usuario");
    } finally {
    ...
    <>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        {u.estado ? (
                          ...
                          <button
                            onClick={() => setUsuarioActivando(u)}
                            className="p-2 rounded-lg transition-colors text-slate-400 hover:text-green-500 hover:bg-slate-700"
        ...
        <div className="px-6 py-4 border-t border-slate-800 bg-slate-900/20 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {Array.from({ length: totalPaginas }, (_, i) => i + 1)
              ...
              .map((item, idx) =>
                item === "..." ? (
                  <span key={`dots-${idx}`} className="text-slate-500 px-1">
                    ...
                  </span>
                ) : (
                  <button
      ...
      {usuarioActivando && (
        <ConfirmActivateModal
          usuario={usuarioActivando}
          onCancelar={() => setUsuarioActivando(null)}
      ...
      {/* ERROR */}
      {error && <ErrorModal mensaje={error} onCerrar={() => setError("")} />}
    </>
  );
}
